...
/** 將既有 Character 轉為 update form 用的 reactive form state（深拷貝陣列與物件以隔離 store 狀態）。 */
export function characterToFormState(character: Character): CharacterUpdateFormState {
  return {
    id: character.id,
    name: character.name,
...
/** 建立 update form 的空 state（找不到角色或測試 fallback 用）。 */
export function createEmptyUpdateFormState(): CharacterUpdateFormState {
  return {
    id: '',
    name: '',
    gender: null,
